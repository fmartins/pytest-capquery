...
This module rigorously proves that the underlying assertion components correctly trap mismatches
across statement quantities, executed query formatting, parameters, and complex nested transactional
savepoints cleanly utilizing the SQLite driver.
"""
...
def test_assert_matches_snapshot_without_manager():
    ...
    with pytest.raises(
        RuntimeError,
        match="SnapshotManager is not configured. Ensure capquery fixture is used correctly.",
    ):
...
def test_assert_matches_snapshot_exceeds_phases(tmp_path):
    ...
    snapshot_manager = SnapshotManager(
        nodeid="test", test_path=tmp_path / "test.py", update_mode=False
    )
